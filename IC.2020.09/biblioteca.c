...
double Temp(double r){//Cálculo da temperatura em função da altitude em K
    ...
    if (alt <= 11000){ //Cálculo da temperatura na Troposfera
        ...
        T = T0 + grad_t*alt; //Comentario: acho que seria mais razoável rever esta função para deixá-la compatível com 
                             //o que usamos no programa main_v3.c, onde tínhamos a temperatura no local do lancamento.
    }
    if ((11000 < alt) && (alt <= 25000)){ //Cálculo da temperatura na baixa estratosfera
        T0 = 216.5;                      //Comentario: se mexermos na parte de cima, vamos ter que adaptar aqui também, para termos 
                                         //a temperatura como uma variável contínua.
        grad_t = 0; //temperatura constante (??)
        T = T0 + grad_t*(alt-11000);
    }
    if (alt > 25000){//Cálculo da temperatura na alta estratosfera
        T0 = 216.5;                     //Comentario: mesma coisa, ao alterar lá em cima, vamos ter que acertar aqui
