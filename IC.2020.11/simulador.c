...
int main(int argc, char* argv[])
{
    ...
    /*if(argc != 9){
        printf("Usar: IC.2020.11.exe altitude latitude(graus) longitude(graus) massa_balao[kg] carga_paga(payload[kg]) empuxo(kgf) Altitude de flutuação(m) Diâmetro da válvula (m)\n\n");
        exit(1);
    }*/
    ...
    while (r - R_T <= 32000){
        ...
        P_h = ro_h*R_h*T_h; //pressão dentro do balão adotando temperatura interna = externa
        ...
        if (cont_p==1000){ // A cada 1000 loops do while os valores das variáveis são impressas no arquivo
