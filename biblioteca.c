...
double Temp(double r){//Cálculo da temperatura em função da altitude em K
    ...
    if (alt <= 11000){ //Cálculo da temperatura na Troposfera
        T0 = 288;
        grad_t = -6.5e-3; //gradiente térmico específico da camada atmosférica em K/m
        T = T0 + grad_t*alt;
    }
...
double raio_b(double P, double T, double m_h, double ro_h0, double r_b){
    double r_bi = pow((3*m_h/(4*M_PI*ro_h0)),1.0/3.0); //raio do balão conforme densidade inicial
    double vol = 4.0*M_PI*pow(r_b,3.0)/3.0;
    double pres_int= R_h*m_h*T/vol; //Pressão interna no balão
